...
def convert_with_midres(midres_module, fits_path: Path, out_dir: Path):
    ...
    if "HiRes" in out_name:
        ...
        out_name = out_name.replace("R10000FITS", "MidRes")
    out_name = re.sub(r"\.fits$", ".txt", out_name, flags=re.IGNORECASE)
    out_path = out_dir / out_name

    old_argv = sys.argv[:]
    try:
        sys.argv = ["openmidres.py", str(fits_path), "--out", str(out_path)]
        midres_module.main()
    finally:
        sys.argv = old_argv
    return out_path

def download_and_process_zip(zip_url: str, work_dir: Path, out_dir: Path, midres_module, overwrite=False):
    ...
    with zipfile.ZipFile(local_zip, "r") as zf:
        ...
        for m in members:
            ...
            with zf.open(m) as src, open(temp_fits, "wb") as dst:
                dst.write(src.read())
            try:
                out_path = convert_with_midres(midres_module, temp_fits, out_dir)
...
def main():
    ap = argparse.ArgumentParser(description="Fetch PHOENIX MedRes R=10000 spectra and convert with your midres.py.")
    ...
    work_dir = Path(args.workdir); work_dir.mkdir(parents=True, exist_ok=True)

    try:
        midres_module = __import__(args.midres_module)
    except Exception as e:
        print(f"Could not import {args.midres_module!r}: {e}", file=sys.stderr)
        sys.exit(1)

    zip_urls = list_all_zip_urls(args.base_url)
    if not zip_urls:
        print("No ZIPs found — check connectivity or the base URL.", file=sys.stderr)
        sys.exit(1)

    print(f"Found {len(zip_urls)} ZIP archives.")
    for u in zip_urls:
        try:
            download_and_process_zip(u, work_dir, out_dir, midres_module, overwrite=args.overwrite)
        except Exception as e:
